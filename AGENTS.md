...
components/
  ...
  ProviderPanel.tsx    # ⚙ 设置 · AI 的「实例设置」页：模板组下拉 / Base URL / 两把 Key / 同步异步 / 实例期参数 / 并发与重试
...
24. **枚举白名单不要写进 DDL，接口形状也不要写进 switch**（2026-09-21 两条相关教训，同日已一并解决）：
    ...
    - 现在的做法（2026-09-22 定稿）：**「模板组 + 接口行 + 能力实例」三张表**（`docs/provider-engine.md`）——`provider_template_group`（一个功能要哪几条接口：kind / 显示名 / 候选模型 / 建议 Base URL）+ `provider_template`（一条接口怎么发、返回从哪取：url / headers / body / **两类入参各一列**（`inst_params_json` 实例参数、`req_params_json` 请求参数，不再用 `vars_json` + `stage` 判别）/ `resp_json` **固定槽位** / decode / 轮询）+ `provider`（实例 = 选哪组模板 + `base_url` + `api_key`/`api_key2` + `mode` + `params_json` + 并发重试）。**role·mode·kind 一律不加 CHECK**，取值由 TS 联合类型 + 保存前 `validateRow/validateGroup` 管。以后接新供应商不改表、不加 switch，也不改代码（模板行就是数据）。**组与接口的 label / note 是用户自己填的单个字符串，不做中英两份**（自定义的东西没法自动翻；界面自身的标题才走 `t()`）。
    - 旧库的形状漂移仍走启动体检：`retireProviderIfStale()` 认**正标志**（不能认「缺了新列」——补列那步 `ensureAllColumns` 会先把新列名塞进旧表，把漂移盖住，实测踩过）：实例侧「还有 `provider_endpoint` 表 / 带 recipe·secrets_json·protocol」，模板侧「`provider_template` 还带 `vars_json` 或行级 `label`」。让位是**改名不删**（用户的模板改动与 Key 都是资产）→ DDL 建新表 → 渲染端 hydrate 先按 seed 铺模板组、再调 `providers.migrate()` 搬回行（`overrides_json` 汇总成 `params_json`；有 async 行则实例 `mode=async`；搬不动的行留在 stale 表等下次）。**改名前要临时 `PRAGMA foreign_keys = OFF`**：外键条款的改写跟这个开关走，开着改父表名会把子表（`provider.tpl_group`）永远指向 `…__stale` 那份死表；同理改名前要把**命名索引**先 DROP（否则新表的 `CREATE INDEX IF NOT EXISTS` 会被静默跳过，丢掉 `ux_provider_active` 这类唯一约束）。回归 `node --experimental-sqlite tools/verify-provider-templates.mjs`（第 5 节专门测这条）。
...
- **供应商配置只在 ⚙ 一处改**，且**实例设置与接口模板是左侧两个独立入口**（不混在同一屏，也不是同一区的页签 —— 日常项与专家项混在一起，结果是没人敢动模板也看不清自己改了什么）：`ProviderPanel`（文案 / 语音 / 图片三类）= 模板组下拉 + Base URL + 两把 Key + 同步异步 + 实例期参数 + 并发/重试；`TemplatesPane`（左侧第 4 项「接口模板」）= 模板组列表 + 每 role 一张接口卡片，可 `＋ 查询接口` / `＋ 音色克隆` 加行、`✕` 删行（确认走 ConfirmHost）、「恢复默认」用 seed 覆盖。**异步的配对靠「同组 + role=query」，不是指针列**；`query` 行是独立卡片，卡片顶部与 `generate·async` 卡片互相点名。
  **配置页的每个控件都必须对得上库里的列**：参数来自该行 `inst_params_json`（`when` 成立才显示），写回 `provider.params_json`；`req_params_json` 声明的是**每次调用由程序给**的正文（text / prompt / wavB64…），配置页不放控件——它只在接口模板页作为「调用时传入」的只读标签出现，因为那是程序的值，不该让人当设置改。
...
**规模**：26 张表 / 4 视图 / **0 触发器** / 698 列（源 `docs/db-schema-v2.sql`，可用 `node --experimental-sqlite` 直接执行验证）。
...
- **★ 新增/改动字段的同步清单（漏一步就会设计↔实现漂移）**：
  ...
  6. 验证（六条全绿才算完）：
     ...
     `node --experimental-sqlite tools/verify-provider-templates.mjs`（**旧库启动 → 三张新表**：让位不删表 + seed 铺组后搬回 Key + 模板组逐字往返 + 真外键拦截 + 异步配对自检）·
