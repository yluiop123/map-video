...
const NOTES = {
  ...
  "provider_template_group": {
    "tpl_group": "模板组 id（一个功能一条）：openai-chat / dashscope-image / custom-tts-1 …",
    "kind": "组所属能力：llm 文案 / tts 语音 / image 图片（取值由 TS 联合类型管，不加 CHECK）",
    "label": "组显示名（L 的 JSON 或纯文本）",
    "note": "组说明（接谁家的哪套端点、有什么坑）",
    "base_url": "新建实例时预填的建议 Base URL",
    "models_json": "候选模型列表 JSON（实例页下拉用）",
    "default_model": "新建实例时预填的模型",
    "default_voice": "新建实例时预填的音色",
    "ord": "组列表排序",
    "created_at": "创建时间（epoch ms，审计用）",
    "updated_at": "最后修改时间（epoch ms，审计用）"
  },
  "provider_template": {
    "tpl_id": "接口行 id，形如 <tpl_group>:<role>:<mode>",
    "tpl_group": "所属模板组",
    "role": "组内用途：generate / synthesize / query / clone（不写 CHECK）",
    "mode": "这条变体服务哪种方式：sync / async（query·clone 恒 sync）",
    "ord": "组内展示顺序",
    "label": "接口显示名（L 的 JSON）",
    "method": "HTTP 方法",
    "url": "地址模板，{baseUrl} 出现在哪由占位符决定",
    ...
    "poll_interval_ms": "异步轮询间隔（离散步长，存原值 ms；只有 query 行读）",
    "poll_timeout_ms": "异步轮询超时（ms；只有 query 行读）",
    "ref_sample_rate": "克隆参考音频要求采样率 Hz（CosyVoice 16k / Qwen-TTS 24k）",
    "created_at": "创建时间（epoch ms，审计用）",
    "updated_at": "最后修改时间（epoch ms，审计用）"
  },
  "provider": {
    ...
    "voice": "音色 / 说话人 ID",
    "speed": "语速（0.5–2）",
    "params_json": "实例期参数值 JSON（模板里 stage=instance 的变量）",
