...
const NOTES = {
  ...
  "narration_entry": {
    ...
    "audio_asset_id": "配音音频：TTS 产物或导入，字节在素材库（项目里不留内联地址）",
  ...
  "music_track": {
    ...
    "name": "曲目名",
    "audio_asset_id": "音频素材（内置曲目在「选用」那一刻也复制进素材库，项目里不留站内路径）",
  ...
  "task": {
    ...
    "provider_id": "用哪个实例发的",
    "project_id": "回填到哪个项目（弱引用：删项目由 removeProjectV2 显式清）",
    "entry_id": "回填到哪条字幕（弱引用：字幕行没了就是「没地方放」，调度器当场跳过）",
